...
package com.dataArtisans.flinkCascading.exec.source;
...
public class CascadingInputFormat implements InputFormat<Tuple, HadoopInputSplit> {
	...
	public void open(HadoopInputSplit split) throws IOException {
		...
		try {
			...
			if(sources.size() != 1) {
				throw new RuntimeException("FlowNode for DataSource may only have a single source");
			}
			FlowElement sourceElement = sources.iterator().next();
			if(!(sourceElement instanceof Tap)) {
				throw new RuntimeException("Source of DataSource must be a Tap");
			}
